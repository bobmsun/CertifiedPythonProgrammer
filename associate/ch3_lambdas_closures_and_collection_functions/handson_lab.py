# 这个 lab 的点是：practice the higher-order function provided by the Python standard library
...
sorted_by_name = sorted(people, key=lambda d: d['name'].lower())         # want to access the key of each d(dictionary) and we want to lower case it before we make the comparison

...
try:
    ...
    print("Error: please provide a name and a repeat count to the scritp")
    sys.exit(1)
...
try:
    ...
    print("Error: the repeat count needs to be an integer")
    sys.exit(1)


# 3) Open a file called `name_repeated.txt` in the `root_files` directory and write a line for each time the name
# repeated. If the user running the script doesn't have write permissions for the directory that
# we're writing to then we might see a PermissionError. PermissionError is new as of 3.3, so let's instead catching
# OSError and IOError to make the script more backwards compatible. If an error is caught print to the screen
...
# chang the owner of the root file directory
